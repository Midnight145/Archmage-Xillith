...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def search_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        try:
            m = await YTDLSource.bot.wait_for('message', check=check, timeout=45.0)
                ...
                if self.autoplay:
                    try:
                        ...
                        async with ctx.typing():
                            try:
                                source = await YTDLSource.create_source(self.bot, ctx, recommended_urls[0], loop=self.bot.loop)
...
class Music(commands.Cog):
    ...
    @commands.command(name='search_music')
    async def _search(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                source = await YTDLSource.search_source(self.bot, ctx, search, loop=self.bot.loop)
            except YTDLError as e:
                await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
            else:
